...
<template>
    <div class="relative">
        <div v-if="!loading">
            <button v-if="isEditing" @click="togglePictogramsMenu()" class="h-9 flex items-center justify-center">
                <img :src="getPictogramImageSrc(step.pictogram, step.elementType)"
                    :alt="getPictogramImageAlt(step.pictogram, step.elementType)" class="h-full" />
                <svg class="w-6 h-6 text-gray-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
            <img v-else :src="getPictogramImageSrc(step.pictogram, step.elementType)"
                :alt="getPictogramImageAlt(step.pictogram, step.elementType)" class="h-9" />
        </div>
        <div v-if="toogleMenu"
            ...
            <div class="py-1 grid grid-cols-5 gap-2 p-2">
                <button v-for="pictogram in pictograms" :key="pictogram.id" @click="selectPictogram(pictogram)"
                    class="focus:outline-none">
                    <img :src="getPictogramImageSrc(pictogram, null)" :alt="getPictogramImageAlt(pictogram, null)"
                        class="h-9" />
                </button>
            </div>
        </div>
